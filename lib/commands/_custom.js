...
async function getMatches(sequence, values) {
  // Get all possible component matches
...
function parseMatches(sequence, values) {
  const setLiteralValue = (obj, i) => {
    let index;
    let v = values.filter((o, i) => {
      let match = o.value === obj.value.name;
      if (match) index = i;
    });
    let next = sequence[i + 1];
    let nextValue = values[index + 1];
    if (next && next.type === 'literal' && nextValue) {
  ...
  for (let i = 0; i < sequence.length; i++) {
    const obj = sequence[i];
    if (obj.type === 'literal' || !obj.values) continue;
    try {
